...
class MemeberRoleController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    public function index($workspaceUuid)
    {
        $workspace = Workspace::findByUuid($workspaceUuid);

        if (!Gate::allows('viewAny', $workspace)) {
            abort(403);
        }
    ...
    public function update(UpdateMemberRoleRequest $request, $uuid)
    {
        ...
        $memberRole->fill([
            ...
            'can_edit_tags' => $validated['can_edit_tags'] ?? $memberRole->can_edit_tags,
